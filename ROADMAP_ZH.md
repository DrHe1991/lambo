...
### 3) 必做：质警经济规则（核心机制）
- **发帖/点赞质押机制（Spend & Earn）**：发布即扣费，好内容赚回来。
- **信誉系统 1.0**：实现 TrustScore (0–1000) 与 UI 颜色等级。
- **影响指数 (Influence Index)**：**彻底移除粉丝数展示**，上线加权影响力算法。
...
## Phase 3（规模化与人类信号堡垒）— 12~24 周

### 1) 治理终极升级（职业陪审团）
- **首席陪审员制度**：高信誉用户可竞聘。
- **治理分成**：职业陪审员分享挑战费用的更高比例（作为失业后的替代收入）。
- **连带责任**：如果裁决被上层推翻，职业陪审员扣除双倍信誉。
...
### 3) 原创对赌机制
- **Pledge 选项**：发帖时勾选“承诺原创 & 非 AI”。
- **惩罚加倍**：违约即面临 200% 的财务与信誉惩罚。
...
> 基于当前 `BitLink/` 目录下的前端代码与 Phase 1 Roadmap 对比，以下是需要完成的开发任务：
